...
const PasswordField = ({ label, value, onChange, show, onToggle, error, hint }) => (
  <div className="flex flex-col gap-1">
    ...
    {hint && !error && <p className="font-dm text-[11px] text-[#9fb0b8]">{hint}</p>}
  </div>
);
    ...
    >
      ...
      >
        ...
        <div className="bg-white flex flex-col justify-center px-8 md:px-10 py-10">
          ...
          <button
            type="button"
            onClick={() => navigate("/doctordashboard/dashboard")}
            className="inline-flex items-center gap-1.5 text-[#6b7f8a] hover:text-[#0e7490] font-dm text-[12px] font-medium transition-colors mb-8 w-fit"
          >
            <FaArrowLeft size={10} />
            Back to Dashboard
          </button>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  ...
                  <div className="flex gap-1 mb-1">
                    {[1, 2, 3, 4].map((i) => (
                      <div
                        ...
                        style={{
                          backgroundColor: strength >= i ? meta.color : "#e5e7eb",
                        }}
                      />
                    ))}
                  </div>
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => navigate("/doctordashboard/dashboard")}
                className="flex-1 font-dm text-[13px] font-semibold text-[#6b7f8a] bg-[#f3f4f6] hover:bg-[#e8eaed] rounded-[12px] py-3 transition-colors border border-transparent"
              >
                Cancel
              </button>
              <button
