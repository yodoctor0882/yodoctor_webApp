...
export default function LabPackages() {
  ...
  const fetchPackages = async () => {
    try {
      ...
      const res = await api.get("/admin/lab/packages", {
        headers: {
      ...
      <div className="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-[#64748B] mt-1">
            Manage all lab test packages and their pricing
          </p>
        </div>
            ...
            <tbody>
              {loading ? (
                ...
                filteredPackages.map((pkg) => {
                  const discount = getDiscount(pkg.price, pkg.mrp);
                  return (
                    <tr
                      key={pkg.id}
                      className="border-t border-[#E2E8F0] hover:bg-[#F8FAFC] transition-colors"
                    >
                      <td className="p-4 text-sm text-[#64748B]">{pkg.id}</td>
                      ...
                      <td className="p-4 text-sm">
                        ...
                        {discount !== null && (
                          <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-[#22C55E]/10 text-[#22C55E]">
                            {discount}% OFF
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex gap-2">
                          <Link
                            to={`/admin/edit-lab-package/${pkg.id}`}
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2 text-sm mb-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg p-3">
                  ...
                  <div>
                    <p className="text-[#64748B] text-xs">MRP</p>
                    <p className="font-medium text-[#94A3B8] text-sm line-through">
                      ₹{pkg.mrp}
                    </p>
                  </div>
                  <div>
                ...
                {discount !== null && (
                  <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-[#22C55E]/10 text-[#22C55E] mb-3">
                    {discount}% OFF
                  </span>
                )}
